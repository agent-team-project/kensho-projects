...
impl Function for StDevP {
    ...
    fn call(&self, args: &[Arg<'_>], ctx: &FnContext<'_>) -> Value {
        ...
        Value::Number((sum_squared_deviations / count).sqrt())
    }
}

static STDEVP: StDevP = StDevP;
inventory::submit! { FunctionEntry(&STDEVP) }
...
mod tests {
    ...
    fn reports_excel_arity_limit() {
        assert_eq!(STDEVP.arity(), (1, Some(255)));
    }
    ...
    fn rejects_wrong_direct_arity() {
        ...
        assert_eq!(STDEVP.call(&args, &fn_ctx), Value::Error(ErrorValue::Value));
    }

    #[test]
    fn calculates_population_standard_deviation_for_known_data() {
        assert_number_close(
            call_values(vec![
                Value::Number(1345.0),
                Value::Number(1301.0),
    ...
    fn aggregates_range_numbers_and_ignores_non_numbers() {
        ...
        assert_number_close(
            STDEVP.call(&[range_arg(&ctx, (0, 0), (2, 1))], &fn_ctx),
            1.632993161855452,
        );
    }

    #[test]
    fn returns_zero_for_one_number() {
        assert_eq!(call_values(vec![Value::Number(5.0)]), Value::Number(0.0));
    }

    #[test]
    fn returns_div0_when_no_numbers_remain() {
        let ctx = TestContext::with_cells(vec![
        ...
        assert_eq!(
            STDEVP.call(&[range_arg(&ctx, (0, 0), (1, 1))], &fn_ctx),
    ...
    fn propagates_aggregation_errors() {
        ...
        assert_eq!(
            STDEVP.call(&[range_arg(&ctx, (0, 0), (0, 1))], &fn_ctx),
