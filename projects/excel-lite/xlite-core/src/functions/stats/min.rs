...
static MIN: Min = Min;
inventory::submit! { FunctionEntry(&MIN) }
...
mod tests {
    ...
    fn reports_excel_arity_limit() {
        assert_eq!(MIN.arity(), (1, Some(255)));
    }
    ...
    fn rejects_wrong_direct_arity() {
        ...
        assert_eq!(MIN.call(&args, &fn_ctx), Value::Error(ErrorValue::Value));
    }

    #[test]
    fn returns_smallest_scalar_number() {
        assert_eq!(
            call_values(vec![
                Value::Number(-3.0),
                Value::Number(7.5),
    ...
    fn aggregates_range_numbers_and_ignores_non_numbers() {
        ...
        assert_eq!(
            MIN.call(&[range_arg(&ctx, (0, 0), (1, 1))], &fn_ctx),
            Value::Number(-7.0)
        );
    }
    ...
    fn returns_zero_for_blank_or_text_only_ranges() {
        ...
        assert_eq!(
            MIN.call(&[range_arg(&blank_ctx, (0, 0), (1, 1))], &blank_fn_ctx),
            Value::Number(0.0)
        );
        ...
        assert_eq!(
            MIN.call(&[range_arg(&text_ctx, (0, 0), (0, 1))], &text_fn_ctx),
            Value::Number(0.0)
        );
    }
