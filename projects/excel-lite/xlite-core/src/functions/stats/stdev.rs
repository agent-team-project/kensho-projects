...
impl Function for StDev {
    ...
    fn call(&self, args: &[Arg<'_>], ctx: &FnContext<'_>) -> Value {
        ...
        Value::Number((sum_squared_deviations / (count - 1.0)).sqrt())
    }
}

static STDEV: StDev = StDev;
inventory::submit! { FunctionEntry(&STDEV) }
...
mod tests {
    ...
    fn reports_excel_arity_limit() {
        assert_eq!(STDEV.arity(), (1, Some(255)));
    }
    ...
    fn rejects_wrong_direct_arity() {
        ...
        assert_eq!(STDEV.call(&args, &fn_ctx), Value::Error(ErrorValue::Value));
    }

    #[test]
    fn calculates_sample_standard_deviation_for_known_data() {
        assert_number_close(
            call_values(vec![
                Value::Number(1345.0),
                Value::Number(1301.0),
    ...
    fn aggregates_range_numbers_and_ignores_non_numbers() {
        ...
        assert_number_close(
            STDEV.call(&[range_arg(&ctx, (0, 0), (2, 1))], &fn_ctx),
            2.0,
        );
    }
    ...
    fn returns_div0_when_fewer_than_two_numbers_remain() {
        ...
        assert_eq!(
            STDEV.call(&[range_arg(&ctx, (0, 0), (1, 1))], &fn_ctx),
            Value::Error(ErrorValue::Div0)
        );
    }
    ...
    fn propagates_aggregation_errors() {
        ...
        assert_eq!(
            STDEV.call(&[range_arg(&ctx, (0, 0), (0, 1))], &fn_ctx),
