...
impl Function for MinIfs {
    ...
    fn call(&self, args: &[Arg<'_>], ctx: &FnContext<'_>) -> Value {
        ...
        for pair in args[1..].chunks_exact(2) {
            ...
            if criteria_range.rows() != min_range.rows() || criteria_range.cols() != min_range.cols()
            {
                return Value::Error(ErrorValue::Value);
            }
        ...
        for row in 0..min_range.rows() {
            for col in 0..min_range.cols() {
                if criteria.iter().all(|(range, criteria_value)| {
                    ctx.matches_criteria(&range.get(row, col), criteria_value)
                }) {
                    match minifs_value(min_range.get(row, col)) {
                        Ok(Some(number)) => {
        ...
        Value::Number(minimum.unwrap_or(0.0))
    }
}

fn minifs_value(value: Value) -> Result<Option<f64>, ErrorValue> {
    match value {
...
static MINIFS: MinIfs = MinIfs;
inventory::submit! { FunctionEntry(&MINIFS) }
...
mod tests {
    ...
    #[test]
    fn reports_minifs_arity() {
        assert_eq!(MINIFS.arity(), (3, Some(255)));
    }
    ...
    #[test]
    fn returns_minimum_matching_cell_for_single_criterion() {
    ...
    #[test]
    fn returns_zero_when_no_cells_match_or_no_numeric_min_cells_match() {
        let ctx = TestContext::with_cells(vec![
            ((0, 0), Value::Number(10.0)),
            ((1, 0), text("ignored")),
    ...
    #[test]
    fn applies_min_range_value_discipline() {
        let ctx = TestContext::with_cells(vec![
            ((0, 0), Value::Number(10.0)),
            ((1, 0), text("ignored")),
    ...
    #[test]
    fn propagates_min_cell_and_criteria_argument_errors() {
        let ctx = TestContext::with_cells(vec![
            ((0, 0), Value::Number(10.0)),
