...
fn ppmt(args: &[Arg<'_>], ctx: &FnContext<'_>) -> Result<f64, ErrorValue> {
    ...
    let interest = interest_payment(rate, per, pv, payment_type, payment)?;

    finite_result(payment - interest)
}
...
fn interest_payment(
    ...
    let interest = balance * rate / denominator;
    finite_result(interest)
}
...
static PPMT: Ppmt = Ppmt;
inventory::submit! { FunctionEntry(&PPMT) }
...
mod tests {
    ...
    fn metadata_matches_contract() {
        assert_eq!(PPMT.name(), "PPMT");
        assert_eq!(PPMT.arity(), (4, Some(6)));
    }
        ...
        assert_close(
            PPMT.call(
                &[
                    ...
                    Arg::Value(Value::Number(2.0 * 12.0)),
                    Arg::Value(Value::Number(2000.0)),
                ],
                &fn_ctx,
            ),
            -75.62318600836664,
