...
static CONCATENATE: Concatenate = Concatenate;
inventory::submit! { FunctionEntry(&CONCATENATE) }
...
fn append_value(
    ...
    let Some(next_chars) = result_chars.checked_add(text_chars) else {
...
mod tests {
    ...
    fn reports_excel_arity_limit() {
        assert_eq!(CONCATENATE.name(), "CONCATENATE");
        assert_eq!(CONCATENATE.arity(), (1, Some(255)));
    }
    ...
    fn rejects_wrong_direct_arity() {
        ...
        let max_args = vec![Arg::Value(Value::Text("x".to_string())); MAX_ARGS];
        let too_many_args = vec![Arg::Value(Value::Text("x".to_string())); MAX_ARGS + 1];

        assert_eq!(
            CONCATENATE.call(&max_args, &fn_ctx),
            Value::Text("x".repeat(MAX_ARGS))
        );
        assert_eq!(
            CONCATENATE.call(&[], &fn_ctx),
            Value::Error(ErrorValue::Value)
        );
        assert_eq!(
            CONCATENATE.call(&too_many_args, &fn_ctx),
            Value::Error(ErrorValue::Value)
        );
    }

    #[test]
    fn concatenates_text_literals() {
        assert_eq!(
            call_values(vec![
    ...
    fn scans_ranges_in_row_major_order() {
        ...
        assert_eq!(
            CONCATENATE.call(&[range_arg(&ctx, (0, 0), (1, 1))], &fn_ctx),
